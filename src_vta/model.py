        ...
        for l in range(num_layers):
            ...
            if is_final_layer:
                if activation == "sigmoid":
                    network_list.append(nn.Sigmoid()) # BCE用 (0~1)
                elif activation == "tanh":
                    network_list.append(nn.Tanh())    # MSE用 (-1~1)
...
class PriorBoundaryDetector(nn.Module):
    ...
    def __init__(self, input_size, output_size=2):
        ...
        # self.network.bias.data = torch.tensor([-0.5, 0.5])
...
class PostBoundaryDetector(nn.Module):
    ...
    def __init__(self, input_size, output_size=2, num_layers=1):
        ...
        # self.network[-1].bias.data = torch.tensor([-0.5, 0.5])
...
class HierarchicalRSSM(nn.Module):
    def __init__(
        ...
        self.dec_obs = Decoder(
            input_size=self.obs_feat_size, feat_size=self.feat_size, activation=dec_activation
...
class VTA(nn.Module):
    ...
    def forward(
        self, obs_data_list, act_data_list, seq_size, init_size, obs_std=1.0, loss_type="bce"
    ):
        # (1) 状態空間モデルの順伝播を実行
        [
            obs_rec_list,
