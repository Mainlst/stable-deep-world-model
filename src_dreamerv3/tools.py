...
class AutoAdapt(nn.Module):
    def __init__(
        ...
        vel=0.1, thres=0.1, inverse=False, device=None
    ):
        super().__init__()
        self._shape = shape
        self._impl = impl
        ...
        # Initialize scale
        scale_val = torch.ones(shape, dtype=torch.float32, device=device)
        if impl == 'fixed':
            scale_val *= scale
        elif impl == 'mult' or impl == 'prop':
             # Official implementation initializes to 1.0 (tf.ones), ignoring scale arg
             pass
        
        # Register as buffer so it's saved with state_dict but no gradients
        self.register_buffer('_scale', scale_val)
    ...
    def update(self, reg):
        if self._impl == 'fixed':
            ...
            # Average over all dimensions except the last len(shape) dimensions
            # If shape is (), average over all.
            # Usually reg is (B, ...) and shape is ().
            if self._shape == ():
                avg = reg.mean()
            else:
                # Assuming shape matches the last dims of reg
                dims_to_reduce = list(range(reg.ndim - len(self._shape)))
                avg = reg.mean(dim=dims_to_reduce)
            ...
            # logic:
            # if above target: increase penalty (scale) to reduce KL
            # if below target: decrease penalty (scale) to allow more KL
            
            # Using current scale from buffer
            current_scale = self._scale
            
            adjusted = (
                above.float() * current_scale * (1 + self._vel) +
                below.float() * current_scale / (1 + self._vel) +
                inside.float() * current_scale
            )
            
            new_scale = torch.clamp(adjusted, self._min, self._max)
            ...
            new_scale = self._scale + self._vel * direction
            new_scale = torch.clamp(new_scale, self._min, self._max)
            self._scale.copy_(new_scale)
        else:
