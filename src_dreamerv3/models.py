...
class ImagBehavior(nn.Module):
    ...
    def _imagine(self, start, policy, horizon):
        ...
        dynamics_type = getattr(self._config, 'dynamics_type', 'rssm')
        if dynamics_type == 'vta':
            return self._imagine_vta(start, policy, horizon, dynamics)
        else:
            return self._imagine_rssm(start, policy, horizon, dynamics)
    
    def _imagine_rssm(self, start, policy, horizon, dynamics):
        """Original RSSM imagination."""
        def step(prev, _):
            state, _, _ = prev
    ...
    def _imagine_vta(self, start, policy, horizon, dynamics):
