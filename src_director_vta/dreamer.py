...
class Dreamer(nn.Module):
    ...
    def _policy(self, obs, state, training):
        ...
        if state is None:
            ...
            director_carry = {
                'step': 0,
                'goal': None, # dec(skill) の結果（g）、Worker への入力
                'skill': None, # Manager の出力（z）、Goal AE の潜在表現
            }
        else:
            latent, action, director_carry = state
        ...
        if self._config.eval_state_mean:
            # Handle both RSSM (stoch) and VTA (obs_stoch) states
            if "obs_stoch" in latent:
                latent["obs_stoch"] = latent["obs_mean"]
        ...
        # VTAの場合は境界情報が必要なので latent も渡す
        if not training:
            action, logprob, director_carry = self._task_behavior.take_action(feat, director_carry, training=False, latent=latent)
        elif self._should_expl(self._step):
            action, logprob, director_carry = self._expl_behavior.take_action(feat, director_carry, training=True, latent=latent)
        else:
            action, logprob, director_carry = self._task_behavior.take_action(feat, director_carry, training=True, latent=latent)
        ...
        state = (latent, action, director_carry)
        return policy_output, state
