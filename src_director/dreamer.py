...
class Dreamer(nn.Module):
    ...
    def _policy(self, obs, state, training):
        ...
        if not training:
            action, logprob, director_carry = self._task_behavior.take_action(feat, director_carry, training=False)
        elif self._should_expl(self._step):
            action, logprob, director_carry = self._expl_behavior.take_action(feat, director_carry, training=True)
        else:
            action, logprob, director_carry = self._task_behavior.take_action(feat, director_carry, training=True)
...
def main(config):
    ...
    wandb_run_name = f"director_{config.task}_{int(config.steps)}"
