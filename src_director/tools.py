...
class Logger:
    def __init__(self, logdir, step, wandb_project=None, wandb_entity=None, wandb_run_name=None, wandb_config=None):
        self._logdir = logdir
    ...
    def write(self, fps=False, step=False):
        ...
        for name, value in self._videos.items():
            ...
            value_tb = value.transpose(1, 4, 2, 0, 3).reshape((1, T, C, H, B * W))
        ...
        if self._wandb_enabled:
            ...
            # images: accept HWC/CHW, grayscale ok
...
class AutoAdapt(nn.Module):
    ...
    - shape: scale parameter shape (e.g., () for scalar)
    - impl: 'fixed' | 'mult' | 'prop'
    - scale: used only when impl == 'fixed' (kept for API parity)
    - target: desired average value of reg (with tolerance thres)
    - min/max: clamp range for scale
    - vel: update velocity
    - thres: tolerance band (e.g., 0.1 -> +/-10%)
    - inverse: if True, flips sign logic (matches original)
    """
    ...
    def __init__(
        ...
        super().__init__()
        self._shape = tuple(shape) if isinstance(shape, (tuple, torch.Size)) else (int(shape),)
        self._impl = impl
        ...
        if self._impl == "fixed":
            # fixed: constant tensor (buffer so it moves with .to(device))
            self.register_buffer("_scale", torch.as_tensor(scale, dtype=dtype).reshape(self._shape))
        elif self._impl in ("mult", "prop"):
            # trainable=False 相当：bufferで保持し、updateでのみ変更
            self.register_buffer("_scale", torch.ones(self._shape, dtype=dtype))
        else:
            raise NotImplementedError(self._impl)

    def forward(self, reg: torch.Tensor, update: bool = True):
        """
        Returns:
          loss: scale * (reg or -reg)
          metrics: dict of tensors (mean/std of reg and scale)
        """
        if update:
            self.update(reg)
        ...
        metrics = {
            "mean": reg.mean(),
            # TFの reduce_std 相当（母標準偏差）に合わせて unbiased=False
            "std": reg.std(unbiased=False),
            "scale_mean": scale.mean(),
            "scale_std": scale.std(unbiased=False),
        }
        return loss, metrics

    def __call__(self, reg: torch.Tensor, update: bool = True):
        # TF版は __call__ を使っているので互換用に残す（forwardへ）
        return super().__call__(reg, update=update)

    def scale(self) -> torch.Tensor:
        # tf.stop_gradient(tf.tensor(scale)) 相当：detach して返す
        return self._scale.detach()

    @torch.no_grad()
    def update(self, reg: torch.Tensor) -> None:
        """
        Update internal scale based on the average of reg.

        TF版：
          avg = reg.mean(list(range(len(reg.shape) - len(self._shape))))
        つまり reg の先頭側の次元を落として、残った shape が self._shape になる想定。
        """
        reg_dim = reg.dim()
        shp_dim = len(self._shape)

        if reg_dim < shp_dim:
            raise ValueError(
                f"reg.dim() ({reg_dim}) must be >= len(shape) ({shp_dim}). "
                f"reg.shape={tuple(reg.shape)}, shape={self._shape}"
            )

        reduce_ndims = reg_dim - shp_dim
        if reduce_ndims > 0:
            dims = tuple(range(reduce_ndims))
            avg = reg.mean(dim=dims)
        else:
            avg = reg

        # avg shape should match self._shape (broadcastable as in TF)
        target = torch.as_tensor(self._target, dtype=avg.dtype, device=avg.device)

        if self._impl == "fixed":
            return

        if self._impl == "mult":
            low = (1.0 / (1.0 + self._thres)) * target
            high = (1.0 + self._thres) * target
            ...
            # adjusted = above*scale*(1+vel) + below*scale/(1+vel) + inside*scale
            s = self._scale.to(device=avg.device, dtype=avg.dtype)
            adjusted = (
                above.to(avg.dtype) * s * (1.0 + self._vel)
                + below.to(avg.dtype) * s / (1.0 + self._vel)
                + inside.to(avg.dtype) * s
            )

            adjusted = torch.clamp(adjusted, min=self._min, max=self._max)
            self._scale.copy_(adjusted.to(dtype=self._scale.dtype))

        elif self._impl == "prop":
            direction = avg - target
            if self._inverse:
                direction = -direction

            s = self._scale.to(device=avg.device, dtype=avg.dtype)
            updated = s + self._vel * direction
            updated = torch.clamp(updated, min=self._min, max=self._max)
            self._scale.copy_(updated.to(dtype=self._scale.dtype))
