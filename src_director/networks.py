...
class MLP(nn.Module):
    def __init__(
        ...
        if self._shape is not None and len(self._shape) == 0:
            self._shape = (1,)
        if dist == 'symlog_disc' and self._shape == (1,):
             self._shape = (255,)
        act = getattr(torch.nn, act)
        self._dist = dist
    ...
    def dist(self, dist, mean, std, shape):
        if dist == "tanh_normal":
            ...
            if shape is not None and isinstance(shape, (tuple, list)):
                # mean: (batch, prod(shape))
                # -> (batch, shape[0], shape[1], ..., shape[-1])
                # E.g., (batch, 8*8) -> (batch, 8, 8)
                mean = mean.reshape(mean.shape[:-1] + tuple(shape))
